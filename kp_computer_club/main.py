...
def login_user(login, password):
    ...
    try:
        ...
        if user:
            ...
            cursor.execute("SELECT Название FROM Роль WHERE id = (SELECT id_роли FROM Клиент_Роль WHERE id_клиента = %s)", (user[0],))
...
def book_computer():
    ...
    try:
        ...
        cursor.execute(
            ...
            """,
            (selected_date, start_time, start_time, end_time, end_time, start_time, end_time, selected_date, start_time, start_time, end_time, end_time, start_time, end_time)
        )
